...
void QuadOpt::initSymmetricAAG()
{
	// define the region of the box
	double cageX = 6.25;
	double cageY = 2.5;
	double cageZ = 2.5;
	// define the nbr of vertices along x direction
	int nbrXItv = 25;
	double itv = cageX / nbrXItv;
	int nbrYItv = cageY / itv;
	vNbrInRow = nbrXItv + 1;
	int vNbrInCol = (nbrYItv + 1) * 1.3; // we hope the patch in the y direction exceeds the shape
	// initialize the variables.
	// the points are initially set in the ceiling.
	int vnbr = vNbrInRow * vNbrInCol;
	V.resize(vnbr, 3);
	verAsVar = std::vector < std::array<bool, 3> >(vnbr, { true, true, true });
	verVarMap = std::vector<std::array<int, 3>>(vnbr, { -1,-1,-1 });
	verVarCount = 0;
	for (int i = 0; i < vNbrInCol; i++)
	{
		for (int j = 0; j < vNbrInRow; j++)
		{
			int vid = i * vNbrInRow + j;
			Eigen::Vector3d pt(itv*j, itv*i, cageZ);
			V.row(vid) = pt;
			if (i == 0)
			{
				if (j == 0 || j == vNbrInRow - 1)
				{
					verAsVar[vid][0] = false;
					verAsVar[vid][1] = false;
					verVarMap[vid][2] = verVarCount;
					verVarCount += 1;
				}
				else
				{
					verAsVar[vid][1] = false;
					verVarMap[vid][0] = verVarCount;
					verVarCount += 1;
					verVarMap[vid][2] = verVarCount;
					verVarCount += 1;
				}
	...
	std::vector<Eigen::Vector3d> Vlist = mat_to_vec_list(V);
	constructRegularF(vnbr, vNbrInRow, F);
	MeshProcessing mp;
	mp.matrix2Mesh(mesh_original, V, F);
	// variables: the vertices, on each vertex there is a normal vector and a binormal vector
	varsize = vnbr * 9;
	getQuadRowColsRegular(vnbr, vNbrInRow, row_front, row_back, col_front, col_back);
	load_diagonal_info(row_front, row_back, col_front, col_back, d0_front, d1_front, d0_back, d1_back);
	std::cout << "Topo obtained\n";
	...
	verUpdate = verOriginal;
	ComputeAuxiliaries = true;
	d0_type = 1; // both the two diagonals are asymptotics
	d1_type = 1;
	Estimate_PG_Angles = false;
	gravity_matrix = var_vec.asDiagonal();
	Eigen::Vector3d vmin(V.col(0).minCoeff(), V.col(1).minCoeff(), V.col(2).minCoeff());
...
void  QuadOpt::assemble_fairness(spMat& H, Eigen::VectorXd& B, Eigen::VectorXd &energy, const std::array<bool, 4>& smoothFamily, const int order)
{
	std::vector<Trip> tripletes;
	int vnbr = V.rows();
	energy = Eigen::VectorXd::Zero(vnbr * 12);//each family takes vnbr * 3 constraints, there are at most 4 families
	...
	for (int i = 0; i < vnbr; i++)
	{
		...
		bool row_smt = smoothFamily[0];
		bool col_smt = smoothFamily[1];
		bool d0_smt = smoothFamily[2];
		bool d1_smt = smoothFamily[3];
		// if (order == 2 && i < vNbrInRow * 2) // cancel the smoothness for the first 2 rows
		// {
		//     row_smt = false;
		// }
...
public:
	...
	int lvx; // center point
	int lvy;
	int lvz;
	int lfx;// front point of a polyline
	int lfy;
	int lfz;
	int lbx; // back point of a polyline
	int lby;
	int lbz;
	int lnx; // normal vector
	int lny;
	int lnz;
	int lrx; // the binormal 0
	int lry;
	int lrz;
	int lrx1; // the binormal 1
	int lry1;
	int lrz1;
	int lrx2; // the binormal 2
	int lry2;
	int lrz2;
	// the row and column front back
	int lrfx;
	int lrfy;
	int lrfz;
	...
	// the diagonal 0 and diagonal 1 front and back
	int ld0fx;
	int ld0fy;
	int ld0fz;
	...
	int d0f, int d0b, int d1f, int d1b, int bnm_start_location, int whichDiagonal)
{
	...
	else
	{
		...
		ld0fx = varOffset * d0f;
		ld0fy = varOffset * d0f + 1;
		ld0fz = varOffset * d0f + 2;

		ld0bx = varOffset * d0b;
		ld0by = varOffset * d0b + 1;
		ld0bz = varOffset * d0b + 2;

		ld1fx = varOffset * d1f;
		ld1fy = varOffset * d1f + 1;
		ld1fz = varOffset * d1f + 2;

		ld1bx = varOffset * d1b;
		ld1by = varOffset * d1b + 1;
		ld1bz = varOffset * d1b + 2;
	}
}
...
void QuadOpt::assemble_normal_conditions_diagonals(spMat& H, Eigen::VectorXd& B, Eigen::VectorXd &energy, const int order) {
	std::vector<Trip> tripletes;
	int vnbr = V.rows();
	energy = Eigen::VectorXd::Zero(vnbr * 3); // todo
	tripletes.reserve(vnbr * 21); // todo
	...
	for (int i = 0; i < vnbr; i++)
	{
		...
		int ld0fx = locator.ld0fx;
		int ld0fy = locator.ld0fy;
		int ld0fz = locator.ld0fz;

		int ld0bx = locator.ld0bx;
		int ld0by = locator.ld0by;
		int ld0bz = locator.ld0bz;

		int ld1fx = locator.ld1fx;
		int ld1fy = locator.ld1fy;
		int ld1fz = locator.ld1fz;
		...
		Eigen::Vector3d Ver(GlobVars[lvx], GlobVars[lvy], GlobVars[lvz]);
		Eigen::Vector3d Vd0f(GlobVars[ld0fx], GlobVars[ld0fy], GlobVars[ld0fz]);
		...
		edge_nm0.push_back(Ver);
		edge_nm1.push_back(Ver + norm);
		double dis0 = (Vd0f - Vd0b).norm();
		double dis1 = (Vd1f - Vd1b).norm();

		// n * (Vd0f - Vd0b) = 0
		tripletes.push_back(Trip(i, lnx, (Vd0f(0) - Vd0b(0)) / dis0));
		tripletes.push_back(Trip(i, lny, (Vd0f(1) - Vd0b(1)) / dis0));
		tripletes.push_back(Trip(i, lnz, (Vd0f(2) - Vd0b(2)) / dis0));

		tripletes.push_back(Trip(i, ld0fx, (norm(0)) / dis0));
		tripletes.push_back(Trip(i, ld0fy, (norm(1)) / dis0));
		tripletes.push_back(Trip(i, ld0fz, (norm(2)) / dis0));

		tripletes.push_back(Trip(i, ld0bx, -(norm(0)) / dis0));
		tripletes.push_back(Trip(i, ld0by, -(norm(1)) / dis0));
		tripletes.push_back(Trip(i, ld0bz, -(norm(2)) / dis0));

		energy[i] = norm.dot(Vd0f - Vd0b) / dis0;
		if (isnan(energy[i]))
		{
			std::cout << "NAN 1\nnorm, " << norm.transpose() << ", values, " << Vd0f.transpose()
				<< ", " << Vd0b.transpose() << ", dis0, " << dis0 << "\n";
		}

		// norm * (Vd1f - Vd1b) = 0
		tripletes.push_back(Trip(i + vnbr, lnx, (Vd1f(0) - Vd1b(0)) / dis1));
		tripletes.push_back(Trip(i + vnbr, lny, (Vd1f(1) - Vd1b(1)) / dis1));
		tripletes.push_back(Trip(i + vnbr, lnz, (Vd1f(2) - Vd1b(2)) / dis1));

		tripletes.push_back(Trip(i + vnbr, ld1fx, (norm(0)) / dis1));
		tripletes.push_back(Trip(i + vnbr, ld1fy, (norm(1)) / dis1));
		tripletes.push_back(Trip(i + vnbr, ld1fz, (norm(2)) / dis1));

		tripletes.push_back(Trip(i + vnbr, ld1bx, -(norm(0)) / dis1));
		tripletes.push_back(Trip(i + vnbr, ld1by, -(norm(1)) / dis1));
		tripletes.push_back(Trip(i + vnbr, ld1bz, -(norm(2)) / dis1));

		energy[i + vnbr] = norm.dot(Vd1f - Vd1b) / dis1;
		// if (isnan(energy[i + vnbr]))
		// {
		//     std::cout<<"NAN 2\n ";
		// }
...
std::vector<int> verIdsOnEdge(const int vnbr, const int vNbrInRow, const int edgeId)
{
	...
	if (edgeId == 0 || edgeId == 2)
	{
		for (int i = 0; i < vNbrInRow; i++)
		{
			...
			{
				result.push_back(i + (vNbrInCol - 1) * vNbrInRow);
			}
		}
	}
	else
	{
		for (int i = 0; i < vNbrInCol; i++)
		{
			if (edgeId == 1)
			{
				result.push_back((i + 1) * vNbrInRow - 1);
			}
			else
			{
				result.push_back(i * vNbrInRow);
			}
		}
	}
	return result;
...
// the constraint is: if the boundary is 0 or 2, symmetric to x-z plane; if is 1 or 3, symmetric to y-z plane
void QuadOpt::assemble_symmetric_boundary_condition(spMat &H, Eigen::VectorXd &B, Eigen::VectorXd &energy, const int whichBnd, const int order)
{
	std::vector<Trip> tripletes;
	int vnbr = V.rows();
	...
	if (whichBnd == 0 || whichBnd == 2)
	{
		planeNormal << 0, 1, 0; // x-z plane
	}
	else
	{
		planeNormal << 1, 0, 0; // y-z plane.
	}
	for (int i = 1; i < edgeVers.size() - 1; i++)
	{
		int vid = edgeVers[i];
		int rf = row_front[vid];
		int rb = row_back[vid];
		...
		int lnz = locator.lnz;
		// the front and back vertices
		int ld0fx, ld0fy, ld0fz, ld0bx, ld0by, ld0bz, ld1fx, ld1fy, ld1fz, ld1bx, ld1by, ld1bz;
		...
		// n * planeNormal = 0
		tripletes.push_back(Trip(i, lnx, planeNormal[0]));
		tripletes.push_back(Trip(i, lny, planeNormal[1]));
		tripletes.push_back(Trip(i, lnz, planeNormal[2]));

		energy[i] = norm.dot(planeNormal);
		...
		tripletes.push_back(Trip(i + edgeVers.size(), ld0fx, planeNormal[0] / dis0));
		tripletes.push_back(Trip(i + edgeVers.size(), ld0fy, planeNormal[1] / dis0));
		tripletes.push_back(Trip(i + edgeVers.size(), ld0fz, planeNormal[2] / dis0));

		tripletes.push_back(Trip(i + edgeVers.size(), ld0bx, -planeNormal[0] / dis0));
		tripletes.push_back(Trip(i + edgeVers.size(), ld0by, -planeNormal[1] / dis0));
		tripletes.push_back(Trip(i + edgeVers.size(), ld0bz, -planeNormal[2] / dis0));

		tripletes.push_back(Trip(i + edgeVers.size(), ld1fx, -signDirection * planeNormal[0] / dis1));
		tripletes.push_back(Trip(i + edgeVers.size(), ld1fy, -signDirection * planeNormal[1] / dis1));
		tripletes.push_back(Trip(i + edgeVers.size(), ld1fz, -signDirection * planeNormal[2] / dis1));

		tripletes.push_back(Trip(i + edgeVers.size(), ld1bx, signDirection * planeNormal[0] / dis1));
		tripletes.push_back(Trip(i + edgeVers.size(), ld1by, signDirection * planeNormal[1] / dis1));
...
void  QuadOpt::assemble_symmetric_deforming_condition(spMat &H, Eigen::VectorXd &B, Eigen::VectorXd &energy, const int order) {
	std::vector<Trip> tripletes;
	int vnbr = V.rows();
	energy = Eigen::VectorXd::Zero(2); // todo
	tripletes.reserve(2); // todo

	// vid = 0, the point should be as high as possible, z approximate to 2.5; vid = vNbrInRow - 1, the point should be as low as possible, z appro 0
	int vid = 0;
	int rf = row_front[vid];
	int rb = row_back[vid];
	int cf = col_front[vid];
...
void QuadOpt::assemble_symmetric_passing_line_condition(spMat &H, Eigen::VectorXd &B, Eigen::VectorXd &energy, const int order)
{
	int vnbr = V.rows();
	...
	for (int i = 0; i < edges.size(); i++)
	{
		...
		locator.getLocationsNormalCondition(order, vnbr, vid, rf, rb,
			cf, cb, d0f, d0b, d1f, d1b, -1, WhichDiagonal);
		// the vertex
		int lv0x = locator.lvx;
		int lv0y = locator.lvy;
		int lv0z = locator.lvz;
		...
		locator.getLocationsNormalCondition(order, vnbr, vid, rf, rb,
			cf, cb, d0f, d0b, d1f, d1b, -1, WhichDiagonal);
		// the vertex
		int lv1x = locator.lvx;
		int lv1y = locator.lvy;
		int lv1z = locator.lvz;

		Eigen::Vector3d Ver0(GlobVars[lv0x], GlobVars[lv0y], GlobVars[lv0z]);
		...
		tripletes.push_back(Trip(i, lv1y, param));
		
		energy[i] = point[1] - 2.5;

		// (1-param)*v0z + param*v1z - 2.5 = 0
		tripletes.push_back(Trip(i + edges.size(), lv0z, 1 - param));
...
void QuadOpt::optSym()
{
	...
	spMat Hnorm;
	Eigen::VectorXd Bnorm;
	assemble_normal_conditions_diagonals(Hnorm, Bnorm, Enorm, order); // the normal vectors
	H += weight_pg * Hnorm;
	B += weight_pg * Bnorm;
	// AAG, the column is the geo, the 2 diagonals are A
	spMat Hbnm;
	Eigen::VectorXd Bbnm;
	int family = -1;
	...
	// deforming condition
	spMat Hdeform;
	Eigen::VectorXd Bdeform;
	assemble_symmetric_deforming_condition(Hdeform, Bdeform, Edefrom, order); 
	H += weight_deform * Hdeform;
	B += weight_deform * Bdeform;

	// fixing the boundary on a line condition
	spMat Hfix;
	Eigen::VectorXd Bfix;
	...
	std::cout << "AAG, ";
	// Esmth, Enorm, Ebnm0, Epg[3], Esym0, Esym1, Edefrom, Efixline;
	double ev_smt = Esmth.norm();
	double ev_norm = Enorm.norm();
	std::cout  << "lap, " << ev_smt << ", normal vector, " << ev_norm << ", ";
